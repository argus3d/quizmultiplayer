...
function alternaFullScreen(_div, _botaoFull, _img1, _img2, _vidDiv, _canvasDiv) {
	...
	if (!document.mozFullScreen && !document.webkitFullScreen) {
		...
		if (mobilecheckOds()) {
			if (_vidDiv && _canvasDiv) {
				videodiv.style.width = "90%";
...
function estrela(nota){
	...
	for (i = 0; i < listas.length; i++) {
		if (divId[i] === undefined) {
			...
			console.log("qual rank" + rankLink);
			console.log(nota.id);
			if (nota.id == "botao1" || nota.id == "botao2") {
				setTimeout(function () {
				...
				result.push(nota1);
				setTimeout(function () {
					$.featherlight.current().setContent($('<div><h2 style="color: white; font-size: 30pt" class="textoCentralizado">Obrigado!</h2></div>'))

				}, 500);
				/*   $.featherlight.current().setContent($(function(){
							var obrigado = document.createElement("h2");
							var textoObg = obrigado.textContent = "Obrigado!";
							console.log(textoObg);
						}, 500)));*/
						setTimeout(function () {
							$.featherlight.close();
							rankLink.remove();
...
/**
 * FUNÇÃO PARA DESCOBRIR O LADO DA IMAGEM INICIAL DO LIVRO E.M.
 * 
 * As imagens de abertura do Ens. Médio possuem dois estilos(https://imgur.com/a/PGjWPfH)
 * e essa função é responsável por descobrir qual lado é, para a posição estar certa
 * 
 * Criado em 17/12/2020
 */
 function handleImagemMedio(el) {
 	const img = new Image()
 	img.src = el.prop('src')
 	...
 	img.onload = () => {
 		...
 		let isLeft = false
 		const {data} = ctx.getImageData(640, 245, 55, 55)
 		...
 		if (isLeft) el.addClass('medioEsquerda')
 	}
}